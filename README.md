...
unique - 지정한 열이 유일한 값을 가져야 한다. 중복될 수 없다,null 허용
primary key - 지정한 값이 null이 아니면서 유일한 값이어야 한다
foreign key - 다른 테이블의 열을 참조하여 존재하는 값만 입력할 수 있습니다.
check - 설정한 조건을 만족하는 데이터만 입력 가능합니다.
```
## 데이터 무결성
```
** 영역 무결성- 열에 저장되는 값의 적정 여부를 확인(자료형,null 등을 정해놓은 범위에 만족하는 데이터임을 규정).
...
UPDATE TABLE_UNIQUE SET LOGIN_ID = 'TEST_ID_04' WHERE LOGIN_ID = NULL;
--필드 이름 변경

CREATE TABLE TABLE_UNIQUE2(
LOGIN_ID VARCHAR2(20) CONSTRAINT LOGIN_ID2 UNIQUE,
LOGIN_PWD VARCHAR2(20) CONSTRAINT LOGIN_PWD2 NOT NULL,
TEL VARCHAR2(20)
);
-- 테이블 생성, 제야조건 이름을 직접 설정
...
ALTER TABLE TABLE_UNIQUE2 MODIFY(TEL CONSTRAINT TEL2 UNIQUE);
--테이블 이름 변경
ALTER TABLE TABLE_UNIQUE2 RENAME CONSTRAINT TEL2 TO TEL_2;
--테이블 이름 변경

ALTER TABLE TABLE_UNIQUE2 DROP CONSTRAINT TEL_2;
--제약 조건 삭제하기
...
LOGIN_PWD VARCHAR(20) CONSTRAINT LOGIN_PWD_PK NOT NULL,
TEL VARCHAR(20)
);
-- 제약 조건 설정 하며 이름을 설정
...
INSERT INTO EMP_PK
VALUES(9999, 'TEST_NNAME', 'TEST_JOB', NULL, TO_DATE('2001/0101', 'YYYY/MM/DD'), 3000, NULL, 10);
-- 데이터 삽입 안됨, 
...
--DEPTNO를 참조하는 데이터가 있어서 삭제 안됨

CREATE TABLE TABLE_CHECK(
LOGIN_ID VARCHAR2(20) CONSTRAINT LOGINID_PK PRIMARY KEY,
LOGIN_PWD VARCHAR(20) CONSTRAINT LOGINPWD_CK CHECK (LENGTH(LOGIN_PWD)>3),
TEL VARCHAR(20)
);
...
LOGIN_ID VARCHAR2(20) CONSTRAINT LOGINID_PK2 PRIMARY KEY,
